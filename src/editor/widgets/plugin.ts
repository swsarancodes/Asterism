...
function isRangeFocused(from: number, to: number, view: EditorView): boolean {
...
export function buildBlockWidgets(view: EditorView): DecorationSet {
  ...
  for (const { from, to } of view.visibleRanges) {
    syntaxTree(view.state).iterate({
      ...
      enter: (node) => {
        ...
        if (name === 'Table') {
          if (!isRangeFocused(nodeFrom, nodeTo, view)) {
...
export const blockWidgetPlugin = ViewPlugin.fromClass(
  class {
    ...
    constructor(view: EditorView) {
      this.decorations = buildBlockWidgets(view);
    }
    ...
    update(update: ViewUpdate) {
      if (
        ...
        this.decorations = buildBlockWidgets(update.view);
      }
    }
  },
  {
